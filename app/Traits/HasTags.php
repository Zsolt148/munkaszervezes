...
trait HasTags
{
    ...
    public function tagsForSelect(): array
    {
        return self::tags()
            ->get()
            ->groupBy('locale')
    ...
    public function tagsForTypeSelect(string $type): array
    {
    ...
    public function tagsWithLocale(string $locale = null): Collection
    {
        return $this->tags->filter(fn (Tag $tag) => $tag->locale === $locale);
    }

    public function attachTags(array|ArrayAccess|Tag $tags, string|null $type = null, string|null $locale = null): static
    {
        $className = static::getTagClassName();

        $tags = collect($className::findOrCreate($tags, $type, $locale));
    ...
    public function attachTag(string|Tag $tag, string|null $type = null, string|null $locale = null)
    {
        return $this->attachTags([$tag], $type, $locale);
    }
    ...
    public function syncTagsWithTypeAndLocale(array|ArrayAccess $tags, string|null $type = null, string|null $locale = null): static
    {
        $className = static::getTagClassName();

        $tags = collect($className::findOrCreate($tags, $type, $locale));

        $this->syncTagIds($tags->pluck('id')->toArray(), $type, $locale);
    ...
    protected function syncTagIds($ids, string|null $type = null, string|null $locale = null, $detaching = true): void
    {
        $isUpdated = false;
        ...
        $current = $this->tags()
            ...
            ->when($type !== null || $locale !== null, function ($query) use ($type, $locale) {
                ...
                return $query->join(
                    ...
                    ->where($tagModel->getTable().'.type', $type) // TODO check if type or locale is null
                    ->where($tagModel->getTable().'.locale', $locale);
            })
            ->pluck('tag_id')
            ->all();
