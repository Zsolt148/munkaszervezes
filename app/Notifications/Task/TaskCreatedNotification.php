...
class TaskCreatedNotification extends Notification implements ShouldQueue
{
    ...
    public function toMail($notifiable)
    {
        return (new MailMessage)
            ->subject("Új feladatod érkezett: {$this->task->name}")
            ->greeting("Kedves {$notifiable->name}!")
            ->line("Új feladatod érkezett: {$this->task->name}")
    ...
    {
        return [
            'title' => "Új feladatod érkezett: {$this->task->name}",
            'body' => "Kedves {$notifiable->name}! Új feladatod érkezett: {$this->task->name}",
            'route' => $this->route,
